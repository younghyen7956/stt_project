...
class SttRepositoryImpl(SttRepository):
    __instance = None
    load_dotenv()
    JSON_FILE_PATH = '/Users/gim-yonghyeon/Documents/GitHub/stt_project/language_dict.json'

    try:
        with open(JSON_FILE_PATH, 'r', encoding='utf-8') as f:
            WHISPER_CODE_TO_NLLB_CODE = json.load(f)
    except FileNotFoundError:
        print(f"오류: '{JSON_FILE_PATH}' 파일을 찾을 수 없습니다. 경로를 확인해 주세요.")
        WHISPER_CODE_TO_NLLB_CODE = {}  # 파일이 없을 경우 빈 딕셔너리로 초기화하거나 오류 처리
    except json.JSONDecodeError:
        print(f"오류: '{JSON_FILE_PATH}' 파일의 JSON 형식이 올바르지 않습니다.")
        WHISPER_CODE_TO_NLLB_CODE = {}
    ...
    def __init__(self):
        ...
        self.nllb_translator_pipeline = None
        self.get_model()
    ...
    def get_model(self):
        ...
        try:
            ...
            nllb_model = AutoModelForSeq2SeqLM.from_pretrained(nllb_checkpoint, torch_dtype=self.torch_dtype).to(
                self.device)
            nllb_tokenizer = AutoTokenizer.from_pretrained(nllb_checkpoint)

            self.nllb_translator_pipeline = pipeline(
                'translation',
                model=nllb_model,
            ...
            print(f"✅ NLLB 번역 모델 ({nllb_checkpoint}) 및 파이프라인 생성 완료.")
        except Exception as e:
            print(f"❌ NLLB 번역 모델 로드 중 오류: {e}")
            self.nllb_translator_pipeline = None

        print("--- SttRepositoryImpl: get_model successfully ---")

    def universal_translation_sync(self, text_to_translate: str, source_lang_code: str, target_lang_code: str) -> str:
        if not self.nllb_translator_pipeline:
            return "NLLB 번역 서비스를 사용할 수 없습니다."
        if not text_to_translate or not text_to_translate.strip():
            return ""

        print(f"  (NLLB 번역 요청: {source_lang_code} -> {target_lang_code})")

        # 소스 및 타겟 언어 코드를 NLLB 형식으로 변환
        nllb_src_lang = self.WHISPER_CODE_TO_NLLB_CODE.get(source_lang_code)
        nllb_tgt_lang = self.WHISPER_CODE_TO_NLLB_CODE.get(target_lang_code)

        if not nllb_src_lang or not nllb_tgt_lang:
            unsupported = source_lang_code if not nllb_src_lang else target_lang_code
            print(f"❌ NLLB 지원하지 않는 언어 코드: {unsupported}")
            return f"NLLB 미지원 언어: {unsupported}"

        try:
            # NLLB 파이프라인을 사용하여 번역 실행
            output = self.nllb_translator_pipeline(
                text_to_translate,
            ...
            print(f"❌ NLLB 번역 중 오류 발생: {e}")
            return f"NLLB 번역 오류: {e}"
    ...
    def generate_llm_response_sync(self, text_input: str, original_language_code: str) -> dict:
        ...
        * During rush hour, Gwanak-ro (the road to the university) can be congested, which may increase the bus travel time slightly."""},
        ...
        try:
            ...
            korean_answer = self.universal_translation_sync(
                original_language_answer,
    ...
    async def transcription(self, audio_file: UploadFile, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if language_info['code'] != 'ko':
                ...
                # 이미 한국어인 경우, 번역 없이 텍스트를 그대로 반환하는 간단한 비동기 함수를 사용
            ...
            # 작업 2: LLM 답변을 생성하는 태스크 (내부적으로 NLLB 번역 포함)
