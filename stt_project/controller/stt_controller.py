...
async def transcribe_audio_endpoint(
        ...
        # [수정 1] enable_translation 파라미터를 API 요청으로 받을 수 있도록 추가합니다.
        enable_translation: bool = Query(
            default=False,
            description="활성화 시 질문과 답변을 한국어로 번역한 결과를 함께 반환합니다."
        ),
    ...
    try:
        print(f"컨트롤러 [/transcribe]: 수신 파일 '{audio_file.filename}', 모델: '{model.value}', 응답 타입: '{response_type.value}', 번역 활성화: {enable_translation}")

        # [수정 2] 서비스 호출 시 enable_translation 값을 전달합니다.
